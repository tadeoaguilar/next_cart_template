...
import {
  ...
  NextHero,
  ImageCld,
} from "@/app/api/utils/contentful/utils";
...
export async function Hero() {
  ...
  const imageDataVar = data.items.map((item) => {
    ...
    return {
      image: String(imgData.public_id) + "." + String(imgData.format),
      order: item.fields.order,
      atext: item.fields.alternateText,
    };
  });
  ...
  return (
    <>
      <div className=" rounded flex flex-row gap-6 mt-6 font-poppins">
        <AspectImage
          src={`https://res.cloudinary.com/${process.env.CLOUDINARY_CLOUD_NAME}/image/upload/c_crop,x_0,y_0,ar_1.4533/next_hero/${image1[0].image}`}
          alt={String(image1[0].atext)}
          format="c_crop,x_0,y_0,ar_1.4533"
          width="872px"
        ...
        <div className="flex flex-col gap-6 ">
          <AspectImage
            ...
            format=""
            width="432px"
            modifier="rounded-[10px] scale-x-[-1]"
            widthAR={432}
            heightAR={288}
