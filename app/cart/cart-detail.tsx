...
export const CartDetail = () => {
  ...
  const [numSubTotal, setNumSubTotal] = useState(0.0);
  ...
  return (
    <>
      {
        <>
          <div className="flex  basis-1/2 items-center justify-center text-3xl font-semibold w-full my-10 ">
            {" "}
            My Shopping Cart
          </div>
          <div className="flex gap-7">
            <div className="flex flex-col basis-4/6 mx-5 border border-gray-next-100 rounded-lg ">
              ...
              {isClient &&
                cart.map((item, index) => {
                  return (
                    <div
                      className="flex flex-row items-center justify-center px-5 text-base font-poppins border-b border-b-gray-next-100"
                      key={item.productId}
                    >
                      ...
                      <div className="  flex basis-1/5 justify-start">
                        <div className="border border-gray-next-100 rounded-2xl p-1">
                          <button
                            className="rounded-full w-8 h-8  bg-gray-next-300 px-2"
                            onClick={() => onClickUpdateCart(item, -1)}
                          >
                            -
                          </button>
                          <label className="px-6 self-center">
                            {" "}
                            {item.quantity}{" "}
                          </label>
                          <button
                      ...
                      <div className=" pl-1 basis-1/5 ">
                        {" "}
                        {`$${(item.quantity * item.price).toFixed(2)} `}
                      </div>
                      <button
                        className="rounded-full w-8 h-8  bg-warning-next px-2 rotate-45"
                        onClick={() => onClickDeleteItem(item.productId)}
                      >
                        +
                      </button>
                    </div>
            ...
            <div className="flex flex-col  basis-2/6 p-6 border border-gray-next-100 rounded-lg">
              <h1 className="from-gray-next-900 font-medium text-xl ">
                Cart Total{" "}
              </h1>
